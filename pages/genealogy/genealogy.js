...
Page({
  ...
  onLoad: async function (options) {
    ...
    this.updateLocationData(); // 在 onLoad 中更新数据
  },

  onShow: function () {
    showTab(this);
    this.updateLocationData(); // 在 onShow 中也更新，确保每次页面显示都是最新的
  },
  ...
  onReady: function () {
    ...
    //确保在页面首次渲染完成后执行
    const app = getApp();
    app.registerLocationCallback((location) => {
      this.setData({
  ...
  onUnload: function () {
    const app = getApp();
    app.unregisterLocationCallback(this.updateLocationData);
  },
  ...
  updateLocationData: function () {
    ...
    if (app.globalData.latitude && app.globalData.longitude) {
      ...
      // 如果 globalData 中还没有定位信息，则显示 "正在定位..."
      this.setData({
        address: '正在定位...'
      });
    }
